...
class _ChatView(QTextEdit):
    """QTextEdit with image click support."""
    ...
    def mousePressEvent(self, event):
        ...
        if fmt.isAnchor():
            ...
            if event.button() == Qt.LeftButton:
                ...
                self.image_right_click.emit(key, event.globalPosition().toPoint())
                return
...
class App(QMainWindow):
    ...
    def _build_left(self, sidebar):
        ...
        log_row_layout.setContentsMargins(16, 0, 16, 16)

        log_btn = QPushButton("📋  运行日志")
        log_btn.setObjectName("icon_btn")
    ...
    def _build_right(self, content):
        ...
        new_chat_btn.setObjectName("icon_btn")
        new_chat_btn.setToolTip("新对话")
        new_chat_btn.clicked.connect(self._new_chat)
        left_btns_layout.addWidget(new_chat_btn)

        save_btn = QPushButton("↓")
        save_btn.setObjectName("icon_btn")
        ...
        self._input_text.setStyleSheet(
            "QTextEdit { background:transparent; border:none; font-family:'Segoe UI'; font-size:10pt; padding:4px; }"
        )
    ...
    def _append_ai_response(self, text, img_bytes):
        ...
        if img_bytes:
            ...
            # wrap in anchor for click detection
            anchor_fmt = QTextCharFormat()
            anchor_fmt.setAnchor(True)
            anchor_fmt.setAnchorHref(img_key)
