...
class App(tk.Tk):
    ...
    def _build_right(self, parent):
        ...
        bottom.pack(fill="x", side="bottom", padx=16, pady=(8, 16))

        # 工具栏行
        toolbar = tk.Frame(bottom, background=C["content"])
        toolbar.pack(fill="x", pady=(0, 8))
        _RoundBtn(toolbar, "＋ 新对话", command=self._new_chat,
                  style="secondary", padx=12, pady=5).pack(side="left")
        _RoundBtn(toolbar, "💾 保存图片", command=self._save_image,
                  style="secondary", padx=12, pady=5).pack(side="left", padx=(6, 0))

        # 胶囊型输入容器（模拟圆角框：外框 Frame + 内嵌 Text）
        input_outer = tk.Frame(bottom, background=C["border"],
                               padx=1, pady=1)   # 1px border via padding
