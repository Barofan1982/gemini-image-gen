...
def _hex_blend(c1: str, c2: str, t: float) -> str:
    """在两个 #RRGGBB 颜色间线性插值，t=0→c1，t=1→c2。"""
...
class _RoundBtn(tk.Canvas):
    """圆角 Canvas 按钮，支持 primary / secondary / ghost / danger / icon 五种风格。"""
    _STYLES = {
        "primary":   ("#007aff", "#ffffff", "#0062cc"),
        "secondary": ("#e8e8ed", "#1d1d1f", "#d1d1d6"),
        "ghost":     ("#f2f2f7", "#007aff", "#dde6f5"),
        "danger":    ("#fff0f0", "#ff3b30", "#ffe0df"),
        "icon":      ("#f2f2f7", "#6e6e73", "#e5e5ea"),
    }

    def __init__(self, parent, text="", command=None,
                 style="secondary", radius=8,
                 font_spec=None, padx=16, pady=7, **kw):
        self._bg, self._fg, self._hov = self._STYLES.get(style, self._STYLES["secondary"])
        self._radius   = radius
        self._label    = text
        self._cmd      = command
        self._disabled = False
        bold = style == "primary"
        self._fnt = font_spec or ("Segoe UI", 10, "bold" if bold else "normal")

        # 用临时 Label 测量文字尺寸
        tmp = tk.Label(parent, text=text, font=self._fnt)
        ...
        try:
            pbg = parent.cget("background")
        except Exception:
            pbg = "#f2f2f7"

        super().__init__(parent, width=W, height=H,
                         highlightthickness=0, borderwidth=0,
                         background=pbg, cursor="hand2", **kw)
        self._draw(self._bg)
    ...
    def _draw(self, fill):
        ...
        if not self._disabled:
            shadow = _hex_blend(pbg, "#000000", 0.10)
            skw = dict(fill=shadow, outline=shadow, width=0)
            self.create_arc(0,     1,     2*r, 2*r+1, start=90,  extent=90, style="pieslice", **skw)
            self.create_arc(w-2*r, 1,     w,   2*r+1, start=0,   extent=90, style="pieslice", **skw)
            self.create_arc(0,     h-2*r+1, 2*r, h+1, start=180, extent=90, style="pieslice", **skw)
            self.create_arc(w-2*r, h-2*r+1, w,   h+1, start=270, extent=90, style="pieslice", **skw)
            self.create_rectangle(r, 1,   w-r, h+1, fill=shadow, outline=shadow)
            self.create_rectangle(0, r+1, w,   h-r+1, fill=shadow, outline=shadow)

        # ── 2. 主体圆角矩形 ───────────────────────────────────
        kw = dict(fill=fill, outline=fill, width=0)
        self.create_arc(0,     0,     2*r, 2*r, start=90,  extent=90, style="pieslice", **kw)
        self.create_arc(w-2*r, 0,     w,   2*r, start=0,   extent=90, style="pieslice", **kw)
        self.create_arc(0,     h-2*r, 2*r, h,   start=180, extent=90, style="pieslice", **kw)
        self.create_arc(w-2*r, h-2*r, w,   h,   start=270, extent=90, style="pieslice", **kw)
        self.create_rectangle(r, 0,   w-r, h,   fill=fill, outline=fill)
        self.create_rectangle(0, r,   w,   h-r, fill=fill, outline=fill)

        # ── 3. 顶部高光（模拟上方来光，增加立体感）────────────
        if not self._disabled:
            hl = _hex_blend(fill, "#ffffff", 0.18)
            hkw = dict(fill=hl, outline=hl, width=0)
            mid = h // 2
            self.create_arc(0,     0,   2*r, 2*r, start=90, extent=90, style="pieslice", **hkw)
            self.create_arc(w-2*r, 0,   w,   2*r, start=0,  extent=90, style="pieslice", **hkw)
            self.create_rectangle(r, 0, w-r, mid, fill=hl, outline=hl)
            self.create_rectangle(0, r, w,   mid, fill=hl, outline=hl)

        # ── 4. 文字 ───────────────────────────────────────────
        fg = "#b0b8c4" if self._disabled else self._fg
        self.create_text(w // 2, h // 2, text=self._label, fill=fg, font=self._fnt)

    def _on_hover(self, entering):
        if not self._disabled:
            self._draw(self._hov if entering else self._bg)
    ...
    def _on_release(self):
        if not self._disabled:
            self._draw(self._bg)
            if self._cmd:
                self.after(10, self._cmd)

    # ── public API ───────────────────────────
    def configure(self, **kw):
        if "state" in kw:
            self._disabled = kw.pop("state") == "disabled"
            self["cursor"] = "" if self._disabled else "hand2"
            self._draw(self._bg)
        if "text" in kw:
...
class _CircleBtn(tk.Canvas):
    """圆形 Canvas 按钮，适合作为 iMessage 风格的发送键。"""
    def __init__(self, parent, text="↑", command=None, size=36,
                 fill="#007aff", fg="#ffffff", hover_fill="#0062cc", **kw):
        try:
            pbg = parent.cget("background")
        except Exception:
            pbg = "#f2f2f7"
    ...
    def _draw(self, fill):
        ...
        # ── 1. 阴影（偏右 1px、偏下 2px）────────────────────
        if not self._disabled:
            shadow = _hex_blend(pbg, "#000000", 0.15)
            self.create_oval(3, 4, s - 1, s, fill=shadow, outline="", width=0)

        # ── 2. 抗锯齿融合环（2 圈，从背景色→主色）────────────
        for i, t in enumerate((0.3, 0.7)):
            blend = _hex_blend(pbg, fill, t)
            d = 1 - i          # 外圈偏大 1px
            self.create_oval(1 - d, 1 - d, s - 1 + d, s - 1 + d,
                             fill=blend, outline="", width=0)

        # ── 3. 主体圆 ─────────────────────────────────────────
        self.create_oval(2, 2, s - 2, s - 2, fill=fill, outline="", width=0)

        # ── 4. 顶部高光椭圆（模拟光从上方照入）──────────────
        if not self._disabled:
            hl = _hex_blend(fill, "#ffffff", 0.25)
            self.create_oval(5, 4, s - 5, s // 2, fill=hl, outline="", width=0)

        # ── 5. 文字 ───────────────────────────────────────────
        fg = "#aaaaaa" if self._disabled else self._fg
        self.create_text(s // 2, s // 2 - 1,
                         text=self._text, fill=fg, font=self._fnt)

    def _on_hover(self, entering):
        if not self._disabled:
            self._draw(self._hover if entering else self._fill)
    ...
    def configure(self, **kw):
        if "state" in kw:
            self._disabled = kw.pop("state") == "disabled"
            self["cursor"] = "" if self._disabled else "hand2"
            self._draw("#888888" if self._disabled else self._fill)
        if kw:
            super().configure(**kw)
...
class App(tk.Tk):
    # ── 配色 ──────────────────────────────────
    C = {
        "bg":          "#f5f5f7",   # 窗口背景（macOS 浅灰）
        "sidebar":     "#f2f2f7",   # 侧边栏
        "content":     "#ffffff",   # 主内容区
        "accent":      "#007aff",   # macOS 蓝
        "accent_dk":   "#0062cc",   # hover 深蓝
        "accent_lt":   "#e8f0fe",   # 浅蓝气泡
        "ai_bubble":   "#f2f2f7",   # AI 气泡
        "border":      "#d1d1d6",   # 边框
        "sep":         "#e5e5ea",   # 分隔线
        "text":        "#1d1d1f",   # 主文字
        "text_sub":    "#8e8e93",   # 次要文字
        "section_hd":  "#6e6e73",   # 小节标题
        "danger":      "#ff3b30",   # 危险色
        "log_bg":      "#161b22",   # 日志背景
        "input_bg":    "#f2f2f7",   # 输入区背景
        "status_bg":   "#1c1c1e",   # 状态栏
        "status_fg":   "#8e8e93",   # 状态文字
    }
    ...
    def __init__(self):
        ...
        self._genai_client = None          # genai.Client 实例（与 session 同生命周期）
        self._session_cfg  = ()            # (model_name, api_size, ratio_text) 建 session 时的参数
    ...
    def _setup_styles(self):
        ...
        s.configure("Sidebar.TLabel", background=C["sidebar"], foreground=C["text"])
        s.configure("Sub.TLabel",   background=C["sidebar"], foreground=C["text_sub"],
                    font=("Segoe UI", 9))
        s.configure("SectionHd.TLabel", background=C["sidebar"],
                    foreground=C["text"],
                    font=("Segoe UI", 10, "bold"))
        s.configure("TPanedwindow", background=C["sep"])

        # Combobox
        s.configure("TCombobox",
                    fieldbackground=C["content"], background=C["content"],
                    foreground=C["text"], arrowcolor=C["text_sub"],
                    bordercolor=C["border"], selectbackground=C["accent_lt"],
                    selectforeground=C["text"], padding=(8, 5))
        s.map("TCombobox",
              fieldbackground=[("readonly", C["content"])],
              bordercolor=[("focus", C["accent"])])

        # Treeview
        s.configure("Treeview",
                    background=C["content"], foreground=C["text"],
                    fieldbackground=C["content"],
                    rowheight=30, font=("Segoe UI", 9),
                    borderwidth=0, relief="flat")
        s.configure("Treeview.Heading",
                    background=C["sidebar"], foreground=C["section_hd"],
        ...
        s.configure("TProgressbar",
                    troughcolor=C["sep"], background=C["accent"], thickness=3)

        # Scrollbar (细 6px，macOS 风格)
        s.configure("TScrollbar",
                    background=C["sep"], troughcolor=C["sidebar"],
                    arrowcolor=C["sidebar"], relief="flat",
                    width=6, arrowsize=0)
    ...
    def _build(self):
        ...
        main = tk.PanedWindow(self, orient="horizontal",
                              ...
                              sashrelief="flat", background=self.C["border"])
        main.pack(fill="both", expand=True, padx=0, pady=(0, 0))

        left  = ttk.Frame(main, style="Sidebar.TFrame")
        right = ttk.Frame(main, style="Content.TFrame")
        # 侧边栏 280px；右侧占剩余所有空间
        main.add(left,  width=280, minsize=220, stretch="never")
        main.add(right, minsize=400, stretch="always")

        self._build_left(left)
        self._build_right(right)
        self._build_log_window()   # 创建隐藏日志浮层

        # 状态栏
        self._status_var = tk.StringVar(value="就绪")
        tk.Label(self, textvariable=self._status_var, anchor="w",
                 background=self.C["status_bg"], foreground=self.C["status_fg"],
    ...
    def _build_left(self, parent):
        C = self.C
        parent.configure(style="Sidebar.TFrame")

        # 可滚动内容区（内容过高时出现滚动条）
        canvas = tk.Canvas(parent, background=C["sidebar"],
                           highlightthickness=0, borderwidth=0)
        ...
        # ── 区块：参考图片 ──────────────────────
        ttk.Label(inner, text="参考图片", style="SectionHd.TLabel"
                  ).pack(anchor="w", padx=16, pady=(20, 8))

        ref_wrap = ttk.Frame(inner, style="Sidebar.TFrame")
        ref_wrap.pack(fill="x", padx=16, pady=(0, 0))

        tree_row = ttk.Frame(ref_wrap, style="Sidebar.TFrame")
        tree_row.pack(fill="x")
        self._ref_tree = ttk.Treeview(tree_row, columns=("type", "file"),
                                       show="headings", height=4)
        self._ref_tree.heading("type", text="参考类型")
        self._ref_tree.heading("file", text="文件名")
        self._ref_tree.column("type", width=80, anchor="w")
        self._ref_tree.column("file", width=100, anchor="w")
        sb_ref = ttk.Scrollbar(tree_row, command=self._ref_tree.yview)
        ...
        ref_btns = ttk.Frame(ref_wrap, style="Sidebar.TFrame")
        ref_btns.pack(fill="x", pady=(6, 0))
        self._ref_count_var = tk.StringVar(value="0 / 14")
        ttk.Label(ref_btns, textvariable=self._ref_count_var,
                  style="Sub.TLabel").pack(side="left")
        _RoundBtn(ref_btns, "清空", command=self._clear_ref,
                  style="danger", padx=8, pady=3
                  ).pack(side="right", padx=(2, 0))
        _RoundBtn(ref_btns, "删除", command=self._remove_ref_image,
                  style="danger", padx=8, pady=3
                  ).pack(side="right", padx=2)
        _RoundBtn(ref_btns, "＋ 添加", command=self._add_ref_image,
                  style="ghost", padx=8, pady=3
                  ).pack(side="right", padx=2)

        # ── 分隔 ────────────────────────────────
        ttk.Separator(inner, orient="horizontal").pack(fill="x", padx=12, pady=(20, 0))

        # ── 区块：画面风格 ──────────────────────
        ttk.Label(inner, text="画面风格", style="SectionHd.TLabel"
                  ).pack(anchor="w", padx=16, pady=(16, 8))

        style_wrap = ttk.Frame(inner, style="Sidebar.TFrame")
        style_wrap.pack(fill="x", padx=16, pady=(0, 0))
        self._style_var = tk.StringVar(value="(无风格)")
        style_values = ["(无风格)"] + list(STYLE_PRESETS.keys())
        ttk.Combobox(style_wrap, textvariable=self._style_var,
                     values=style_values, state="readonly").pack(fill="x")
        self._style_label = ttk.Label(style_wrap, text="不附加任何风格提示词",
                                      style="Sub.TLabel", wraplength=200, justify="left")
        self._style_label.pack(anchor="w", pady=(4, 0))
        ...
        # ── 区块：生成参数 ──────────────────────
        ttk.Label(inner, text="生成参数", style="SectionHd.TLabel"
                  ).pack(anchor="w", padx=16, pady=(16, 8))

        param_frame = ttk.Frame(inner, style="Sidebar.TFrame")
        param_frame.pack(fill="x", padx=16, pady=(0, 8))
        params = [
            ("模型",     "_model_var",  list(MODELS.keys())),
            ("分辨率",   "_res_var",    RESOLUTIONS),
            ("比例",     "_ratio_var",  ASPECT_RATIOS_FLASH),
            ("格式",     "_format_var", OUTPUT_FORMATS),
            ("数量",     "_count_var",  ["1 张", "2 张", "3 张", "4 张"]),
        ]
        for i, (label, attr, values) in enumerate(params):
            ttk.Label(param_frame, text=label, style="Sidebar.TLabel"
                      ).grid(row=i, column=0, sticky="e", pady=4, padx=(0, 8))
            var = tk.StringVar(value=values[0])
            setattr(self, attr, var)
            cb = ttk.Combobox(param_frame, textvariable=var, values=values,
                              state="readonly", width=16)
            cb.grid(row=i, column=1, sticky="ew", pady=4)
            if attr == "_ratio_var":
                self._ratio_combo = cb
        ...
        log_row = ttk.Frame(inner, style="Sidebar.TFrame")
        log_row.pack(fill="x", padx=16, pady=(0, 16))
        _RoundBtn(log_row, "📋  运行日志", command=self._toggle_log,
                  style="icon", padx=10, pady=4).pack(side="left")
    ...
    def _build_right(self, parent):
        ...
        self._progress.pack(fill="x", side="top", padx=0, pady=(0, 0))

        # ── iMessage 输入区（底部固定）─────────
        bottom = tk.Frame(parent, background=C["content"])
        bottom.pack(fill="x", side="bottom", padx=16, pady=(4, 16))

        # 胶囊型输入容器：[左侧图标] [输入框] [发送按钮]
        input_outer = tk.Frame(bottom, background=C["border"],
                               padx=1, pady=1)   # 1px border
        input_outer.pack(fill="x")
        input_inner = tk.Frame(input_outer, background=C["input_bg"])
        input_inner.pack(fill="x")

        # 左侧：新对话 + 保存图片（小圆形图标按钮）
        left_btns = tk.Frame(input_inner, background=C["input_bg"])
        left_btns.pack(side="left", padx=(8, 2), pady=8)
        _CircleBtn(left_btns, "＋", command=self._new_chat,
                   size=30, fill="#e8e8ed", fg="#1d1d1f", hover_fill="#d1d1d6"
                   ).pack(pady=(0, 4))
        _CircleBtn(left_btns, "↓", command=self._save_image,
                   size=30, fill="#e8e8ed", fg="#1d1d1f", hover_fill="#d1d1d6"
                   ).pack()

        # 中间：输入文本
        self._input_text = tk.Text(input_inner, wrap="word",
                                   font=("Segoe UI", 10),
                                   background=C["input_bg"], foreground=C["text"],
                                   insertbackground=C["accent"],
                                   relief="flat", borderwidth=0,
                                   padx=10, pady=10, height=3)
        self._input_text.pack(side="left", fill="both", expand=True)

        # 右侧：圆形发送按钮
        send_wrap = tk.Frame(input_inner, background=C["input_bg"])
        send_wrap.pack(side="right", padx=(4, 8), pady=8)
        self._send_btn = _CircleBtn(send_wrap, text="↑",
                                    command=self._send_message,
                                    size=36,
                                    fill=C["accent"],
                                    hover_fill=C["accent_dk"])
        self._send_btn.pack()
        ...
        # ── 对话历史（填满剩余空间）──────────────
        chat_outer = tk.Frame(parent, background=C["content"])
        chat_outer.pack(fill="both", expand=True, padx=0, pady=(0, 0))

        self._chat_text = tk.Text(chat_outer, state="disabled", wrap="word",
                                  background=C["content"], cursor="arrow",
                                  relief="flat", borderwidth=0,
                                  font=("Segoe UI", 10), padx=16, pady=12,
                                  spacing1=2, spacing3=6)
        sb_chat = ttk.Scrollbar(chat_outer, command=self._chat_text.yview)
        ...
        self._chat_text.tag_configure("user_msg",
            foreground=C["text"], lmargin1=20, lmargin2=20,
            background=C["accent_lt"], spacing1=4, spacing3=4)
        self._chat_text.tag_configure("ai_name",
            foreground="#059669", font=("Segoe UI", 9, "bold"))
        self._chat_text.tag_configure("ai_msg",
            foreground=C["text"], lmargin1=20, lmargin2=20,
            background=C["ai_bubble"], spacing1=4, spacing3=4)
        self._chat_text.tag_configure("divider",
            foreground=C["text_sub"], font=("Segoe UI", 8),
            justify="center", spacing1=8, spacing3=8)
        self._chat_text.tag_configure("img_hint",
            foreground=C["text_sub"], font=("Segoe UI", 8))
    ...
    def _build_log_window(self):
        ...
        win.configure(background=C["log_bg"])
        win.resizable(True, True)
        win.protocol("WM_DELETE_WINDOW", win.withdraw)

        self._log_text = tk.Text(win, state="disabled", wrap="word",
                                 background=C["log_bg"], foreground="#c9d1d9",
                                 font=("Cascadia Code", 10)
                                      if self._font_exists("Cascadia Code")
                                      else ("Consolas", 10),
                                 relief="flat", borderwidth=0, padx=10, pady=8)
        sb_log = ttk.Scrollbar(win, command=self._log_text.yview)
        ...
        self._log_text.tag_configure("time",  foreground="#484f58")

        win.withdraw()   # 默认隐藏
        self._log_win = win

    def _toggle_log(self):
        if not hasattr(self, "_log_win") or self._log_win is None:
            return
        if self._log_win.winfo_viewable():
            self._log_win.withdraw()
        else:
            # 定位到主窗口右下角附近
            wx = self.winfo_x() + self.winfo_width() - 556
            wy = self.winfo_y() + self.winfo_height() - 316
            self._log_win.geometry(f"540x300+{max(0, wx)}+{max(0, wy)}")
    ...
    def _on_model_change(self, *_):
        """切换模型时更新比例下拉列表"""
        ratios = MODEL_RATIOS.get(self._model_var.get(), ASPECT_RATIOS_FLASH)
        self._ratio_combo.configure(values=ratios)
        if self._ratio_var.get() not in ratios:
            self._ratio_var.set(ratios[0])  # 重置为 auto
    ...
    def _on_style_change(self, *_):
        ...
        if key and key != "(无风格)":
            ...
            self._style_label.configure(text="不附加任何风格提示词")
    ...
    def _send_message(self):
        ...
        if style_desc:
            full_prompt += f"\n\n风格要求：{style_desc}"
        ratio_text = self._ratio_var.get().split()[0]   # "auto" / "16:9" 等
    ...
    def _on_one_image(self, img_bytes: bytes | None, text: str | None,
        ...
        if img_bytes:
            ...
            # 512：API 原生支持，若实际返回偏差则修正；4K：API 不支持，放大兜底
            target_long = {"512": 512, "1K": 1024, "2K": 2048, "4K": 3840}.get(self._res_var.get(), 0)
            ...
            if target_long > 0 and long_side != target_long:
                ...
                self._log(f"已将图像{action}：{long_side}px → {target_long}px（长边，{self._res_var.get()}）")
    ...
    def _restore_session(self, state: dict):
        ...
        self._session_cfg  = ()
        self._current_turns.clear()
        self._session_ts = ""          # 留空，首次生成时再分配新 ts
        self._last_image_bytes = None
