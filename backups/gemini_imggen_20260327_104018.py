...
class App(tk.Tk):
    ...
    def _send_message(self):
        if not self._api_key:
            messagebox.showwarning("未解锁", "请先完成 API Key 设置")
            return
    ...
    def _add_ref_image(self):
        if len(self._ref_images) >= self.MAX_REF_IMAGES:
            messagebox.showwarning("已达上限", f"最多只能添加 {self.MAX_REF_IMAGES} 张参考图")
            return
        ...
        for path in paths:
            if len(self._ref_images) >= self.MAX_REF_IMAGES:
                messagebox.showwarning("已达上限",
                    f"已添加至上限 {self.MAX_REF_IMAGES} 张，剩余文件未添加")
                break
    ...
    def _save_image(self):
        ...
        if not self._last_image_bytes:
            messagebox.showinfo("提示", "还没有生成图片")
            return
